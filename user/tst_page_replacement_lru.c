...
void _main(void)
{
	...
	//cprintf("Checking PAGE LRU algorithm... \n");
	{
		...
		if( ROUNDDOWN(myEnv->__uptr_pws[5].virtual_address,PAGE_SIZE) !=  0x807000)  panic("Page LRU algo failed.. trace it by printing WS before and after page fault");
		...
		if(myEnv->page_last_WS_index != 5) panic("wrong PAGE WS pointer location %d", myEnv->page_last_WS_index);
