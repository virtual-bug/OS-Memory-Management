...
//2017
#define ACTUAL_START (KERNEL_HEAP_START + KERNEL_SHARES_ARR_INIT_SIZE + KERNEL_SEMAPHORES_ARR_INIT_SIZE)
...
int test_kmalloc_nextfit()
{
	...
	if ((pf_calculate_free_frames() - freeDiskFrames) != 0) panic("Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
	if ((sys_calculate_free_frames() - freeFrames) != ((10*2*Mega) - PAGE_SIZE - KERNEL_SHARES_ARR_INIT_SIZE - KERNEL_SEMAPHORES_ARR_INIT_SIZE)/PAGE_SIZE) panic("Wrong free: Extra or less pages are removed from main memory");
...
int test_kheap_phys_addr()
{
	...
	//test kheap_physical_address after kmalloc only [40%]
	{
		...
		uint32 allPAs[(13*Mega + 32*kilo+KERNEL_SHARES_ARR_INIT_SIZE+KERNEL_SEMAPHORES_ARR_INIT_SIZE)/PAGE_SIZE] ;
		i = 0;
...
int test_kheap_virt_addr()
{
	...
	uint32 allPAs[(13*Mega + 32*kilo+KERNEL_SHARES_ARR_INIT_SIZE+KERNEL_SEMAPHORES_ARR_INIT_SIZE)/PAGE_SIZE] ;
	int numOfFrames = (13*Mega + 32*kilo+KERNEL_SHARES_ARR_INIT_SIZE+KERNEL_SEMAPHORES_ARR_INIT_SIZE)/PAGE_SIZE ;
	...
	//test kheap_virtual_address after kmalloc and kfree [40%]
	{
		...
		uint32 startIndex = (KERNEL_SHARES_ARR_INIT_SIZE+KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE;
